    ...
    <div class="col-sm-12 col-md-12 center-block" style="float: none;">
        ...
        if($my == "edit_submit"){
           $id = intval($_GET['id']);
            ...
            $sql = "update ayangw_goods set {$set} where id = ".$id;
            ...
            $id = intval($_GET['id']);
            $grow = $DB->get_row("select * from ayangw_goods where id = ".$id);
$uptypes=array(
    'image/jpg',
    'image/jpeg',
    'image/png',
 ...
 -->
	...
	<div class="form-group">
	  ...
	  <input type="submit" name="" id="" value="保存商品" class="btn btn-primary form-control"/><br/>
            ...
            if($DB->insert_array("ayangw_goods",$garr)){
...
<?php 
	...
	$numrows=$DB->count("SELECT count(*) from ayangw_goods");
	$sjz= $DB->count("SELECT count(*) from ayangw_goods where state = 1");
        $xjz= $DB->count("SELECT count(*) from ayangw_goods where state = 0");
     $sql=" 1";
...
if(!empty($_GET['act']) && $_GET['act'] != null && $_GET['act'] == "sousuo"){
    $pz = $_POST['pz'];
    $sql = "SELECT * FROM ayangw_goods WHERE  gName like '%$pz%' or id like '%$pz%' or tpId  like '%$pz%'   order by id desc ";
}else{
    if(!empty($_GET['cid']) && intval($_GET['cid']) > 0){
        $sql = "SELECT * FROM ayangw_goods WHERE tpId = ".intval($_GET['cid'])." order by id desc limit $offset,$pagesize";
    }else{
          $sql = "SELECT * FROM ayangw_goods WHERE {$sql} order by id desc limit $offset,$pagesize";
    }
